    ...
    public function sendMail($data,$pdf_name)
{
    ...
    $fileName = $pdf_name; // Set the filename for the PDF attachment
    // $pdfContent = 'Please find the attached SRS PDF.'; // Replace this with the actual content of the PDF


    Mail::raw('Please find the attached SRS PDF.', function ($message) use ($recipient, $subject, $fileName) {
        ...
        $message->to($recipient) // Use the $recipient variable instead of 'recipient'
            ...
            ->attachData($fileName, ['mime' => 'application/pdf']); // Use the $fileName variable instead of $content['fileName']
    });
